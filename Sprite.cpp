...
{
    if (!hDC || m_pBitmap == NULL || m_bHidden)
        return;
    ...
    if (m_pBitmap != NULL && !m_bHidden)
    {
        ...
        // Çerçevenin sprite sheet'teki konumunu hesapla
        int xPart = (m_iCurFrame % 9) * frameWidth; // Sütun
        int yPart = (m_iCurFrame / 9) * frameHeight; // Satır

        // Çerçeveyi çiz
        m_pBitmap->DrawPart(hDC, m_rcPosition.left, m_rcPosition.top,
            xPart, yPart, frameWidth, frameHeight, TRUE);
    }
