...
BOOL Bitmap::Create(HDC hDC, LPTSTR szFileName)
{
    ...
    {
        MessageBox(NULL, TEXT("Bitmap dosyasý açýlamadý!"), szFileName, MB_OK | MB_ICONERROR);
        return FALSE;
    }
    ...
    if ((!bOK) || (dwBytesRead != sizeof(BITMAPFILEHEADER)) ||
        (bmfHeader.bfType != 0x4D42)) // 'BM' kontrolü
    {
        MessageBox(NULL, TEXT("Bitmap dosyasý geçerli deðil!"), szFileName, MB_OK | MB_ICONERROR);
        CloseHandle(hFile);
        return FALSE;
    }
    ...
    if (pBitmapInfo != NULL)
    {
        ...
        {
            MessageBox(NULL, TEXT("Bitmap bilgi baþlýðý okunamadý!"), szFileName, MB_OK | MB_ICONERROR);
            CloseHandle(hFile);
            Free();
            return FALSE;
        }
    ...
    MessageBox(NULL, TEXT("Bitmap yüklenirken bir hata oluþtu!"), szFileName, MB_OK | MB_ICONERROR);
    Free();
    return FALSE;
}
